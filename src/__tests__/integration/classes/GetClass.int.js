...
describe("Get Class", () => {
  it("given request from teacher, returns class and 200", async () => {
    ...
    const { id, name, owner, roster, droppedStudents, joinCode, isArchived } =
      res.body
    ...
    expect(
      roster.some(
        (seat) => seat.student === rosteredStudent.student.toHexString(),
      ),
    ).toBe(true)
    expect(
      droppedStudents.some(
        (seat) => seat.student === droppedStudent.student.toHexString(),
      ),
    ).toBe(true)
  })
