...
async function handleJoinClass({ classId, studentId }) {
    ...
    assignments.forEach(async (assignment) => {
        const responseCheck = await responseRepo.findOne({assignment: assignment._id, owner: studentId})
        if(responseCheck) {
            return
        }
        await createResponseCase.execute({ assignmentId: assignment._id.toHexString(), owner: studentId })
    })
}

module.exports = EventBus.subscribe(EVENTS.JoinClass, async (payload) => {await handleJoinClass(payload)})
