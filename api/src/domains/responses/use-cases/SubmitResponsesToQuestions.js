...
module.exports = class SubmitResponsesToQuestions extends UseCase {
  ...
  async execute({ responseId, responses }) {
    ...
    const activityContent = await this.repository.getActivityContent(
      assignment.activity.toHexString(),
    )
    ...
    const questionsToCheck = activityContent.questions
      .map((question) =>
    ...
    for (const response of responses) {
      ...
      for (const question of questionsToCheck) {
        if (question.id === response.question) {
          for (const condition of question.conditions) {
            if (
              response.content === condition.expression &&
      ...
      const responseUpdate = assignmentResponse.responses
        .map((assignmentResponse) => {
          if (response.question === assignmentResponse.question) {
            const score = questionsToCheck.find(
    ...
    assignmentResponse.responses = assignmentResponse.responses.map(
      (assignmentResponse) => {
    ...
    const updatedAssignmentResponse = await this.repository.updateResponses({
      responseId: responseId,
      responses: assignmentResponse.responses,
    })
