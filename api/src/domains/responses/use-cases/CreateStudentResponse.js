...
module.exports = class CreateAssignmentResponseUseCase extends UseCase {
  ...
  async execute({ assignmentId, owner }) {
    ...
    const assignmentResponseVariables =
      activityContent.variables.length > 0
    ...
    const assignmentResponseQuestions =
      activityContent.questions.length > 0
        ...
        : []
    const assignmentResponseProps = AssignmentResponse.toDb({
      assignment: assignmentId,
      owner: owner,
      variables: assignmentResponseVariables,
      responses: assignmentResponseQuestions,
    })
    const assignmentResponse = await this.repository.create(
      new AssignmentResponse(assignmentResponseProps),
    )
