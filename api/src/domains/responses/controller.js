...
module.exports = class AssignmentResponseController {
  constructor(repository) {
    ...
    this.submitResponsesToQuestions = this.submitResponsesToQuestions.bind(this)
  }

  async create(req, res, next) {
    try {
      const createResponseCase = new CreateAssignmentResponseUseCase(
        this.repository,
      )
  ...
  async getResponseForStudent(req, res, next) {
    try {
      const getResponseForStudentCase = new GetResponseForStudent(
        this.repository,
      )
  ...
  async submitResponsesToQuestions(req, res, next) {
    try {
      const submitResponsesToQuestionsCase = new SubmitResponsesToQuestions(
        this.repository,
      )
      const { responseId } = req.params
      const { responses } = req.body
      const data = { responseId: responseId, responses: responses }
      const result = await submitResponsesToQuestionsCase.execute(data)
      res.status(200).send(result)
    } catch (err) {
      next(err)
    }
